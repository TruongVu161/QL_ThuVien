    ...
    public class StoreContext
    {
        ...
        public int CreateSach(Sach s)
        {
            using (MySqlConnection conn = GetConnection())
            {
                ...
                {
                    conn1.Open();
                    var str1 = "select MaSach from sach where MaSach=@masach";
                ...
                {
                    conn1.Open();
                    var str1 = "select TenSach from sach where TenSach=@tensach";
                    MySqlCommand cmd1 = new MySqlCommand(str1, conn1);
                    cmd1.Parameters.AddWithValue("tensach", s.TenSach);
                    using (var reader = cmd1.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            tsach = reader["TenSach"].ToString();
                        }
                    }
                    if (tsach == s.TenSach)
                        return 200;
        ...
        public int CreateDocGia(DocGia dg)
        {
            using (MySqlConnection conn = GetConnection())
            {
                ...
                {
                    conn1.Open();
                    var str1 = "select MaDG from docgia where MaDG=@madg";
                    MySqlCommand cmd1 = new MySqlCommand(str1, conn1);
                    cmd1.Parameters.AddWithValue("madg", dg.MaDG);
                    using (var reader = cmd1.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            mdocgia = reader["MaDG"].ToString();
                        }
                    }
                    if (mdocgia == dg.MaDG)
                        return 100;
        ...
        public int CreatePhieuMuon(PhieuMuon pm)
        {
            using (MySqlConnection conn = GetConnection())
            {
                ...
                {
                    conn1.Open();
                    var str1 = "select MaPM from phieumuon where MaPM=@maphieumuon";
                    MySqlCommand cmd1 = new MySqlCommand(str1, conn1);
                    cmd1.Parameters.AddWithValue("maphieumuon", pm.MaPM);
                    using (var reader = cmd1.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            maphieumuon = reader["MaPM"].ToString();
                        }
                    }
                    if (maphieumuon == pm.MaPM)
                        return 200;
                }
                return (cmd.ExecuteNonQuery());             
            }
